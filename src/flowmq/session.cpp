...
Session::Session(tcp::socket&& socket)
    : socket_(std::move(socket)), status_(CONNECTED) {}
...
void Session::write_message(std::shared_ptr<Message> msg_copy) {
  ...
  boost::asio::async_write(
      socket_,
      boost::asio::buffer(msg_copy->header(),
...
void Session::read_header() {
  ...
  boost::asio::async_read(
      socket_,
      boost::asio::buffer(read_message_buffer_.header(),
                          read_message_buffer_.header_length()),
      [this, self](boost::system::error_code error, std::size_t /*length*/) {
        if (!error) {
          // this is inovked in the io_context thread, read_message_buffer is
          // invoked without any synchronization, if multiple threads are enable
          // for the associated io_context, application must manage the
...
void Session::read_body() {
  ...
  boost::asio::async_read(
      socket_,
      boost::asio::buffer(read_message_buffer_.body(),
                          read_message_buffer_.body_length()),
      [this, self](boost::system::error_code error, std::size_t /*length*/) {
        if (!error) {
          // LOG_INFO << "session : obtained message : " <<
          // std::string(read_message_buffer_.body(),
          // read_message_buffer_.body_length()) << '\n';
