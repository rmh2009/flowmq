...
TEST(LogEntryStorage, TestWriteAndRead1000) {
  ...
  EXPECT_EQ(0,
            LogEntryStorage::save_log_entry_to_file(TEST_FILE_NAME, entries));
  ...
  for (int i = 0; i < 1000; ++i) {
    EXPECT_TRUE(entries[i].SerializeAsString() ==
                loaded_entries[i].SerializeAsString());
  }

  EXPECT_EQ(0,
            LogEntryStorage::append_log_entry_to_file(TEST_FILE_NAME, entries));
...
TEST(LogEntryStorage, TestWriteAndReadMetadata) {
  ...
  EXPECT_EQ(
      0, MetadataStorage::save_metadata_to_file(TEST_FILE_NAME_META, metadata));
  decltype(metadata) loaded_metadata;

  MetadataStorage::load_metadata_from_file(TEST_FILE_NAME_META,
                                           &loaded_metadata);
