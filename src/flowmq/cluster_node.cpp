...
ClusterNode::ClusterNode(
  ...
  LOG_INFO << "after loading data: commit index is : " << commit_index_
           << ", log entry size: " << log_entries_.size() << '\n';
}

void ClusterNode::add_log_entry(const LogEntry entry) {
  // this should always be run in io_context, client should not directly call
  // this
...
void ClusterNode::start_send_hearbeat() {
  heartbeat_timer_.expires_from_now(
      boost::posix_time::seconds(HEARTBEAT_EXPIRE_SECONDS));
  ...
  heartbeat_timer_.async_wait([this](boost::system::error_code) {
    if (state_ == LEADER) {
      ...
      //            {cur_term_, node_id_, last_log_index, last_log_term, {},
      //            commit_index_}));

      cluster_master_->cluster_manager()->broad_cast(
          serialize_raft_message(msg));
      for (int i = 0; i < total_nodes_; ++i) {
        if (i == node_id_) continue;
...
void ClusterNode::start_vote_scheduler() {
  ...
  vote_timer_.async_wait([this](boost::system::error_code) {
    ...
    if (state_ == FOLLOWER &&
        last_heart_beat_received_ + HEARTBEAT_EXPIRE_SECONDS + 2 <
            now) {  // 2 is the margin in case there was a delay in receiving
                    // heartbeats
      state_ = CANDIDATE;
    }
    ...
    if (state_ == CANDIDATE) {
      ...
      timer->async_wait([timer, this](boost::system::error_code const&) {
        ...
        cluster_master_->cluster_manager()->broad_cast(
            serialize_raft_message(raft_msg));
      });
    }
...
void ClusterNode::start_statistics_scheduler() {
  ...
  stats_timer_.async_wait([this](boost::system::error_code) {
    std::stringstream ss;
    ss << "\n_________________ Printing statistics for partition "
       << partition_id_ << " ______________________\n";

    // printing entries
    for (auto entry : log_entries_) {
      LOG_DEBUG << entry.index() << ',' << entry.operation() << ','
                << entry.message();
    }
...
void ClusterNode::message_handler(const Message& msg) {
  ...
  boost::asio::post(io_context_, std::bind(&ClusterNode::local_message_handler,
                                           this, std::move(raft_msg)));
}
...
void ClusterNode::local_message_handler(RaftMessage raft_msg) {
  ...
  switch (raft_msg.type()) {
    case RaftMessage::APPEND_ENTRIES_REQUEST: {
      ...
      // check if term is up-to-date, if not send back current term, leader
      // should update its term.
      if (req.term() < cur_term_) {
        LOG_INFO << "WARNING: Received expired heart beat from leader! sending "
                    "back current term \n";
        cur_leader_ = -1;
        ...
        // msg.loadAppendEntriesResponse(AppendEntriesResponseType({cur_term_,
        // node_id_, 0, -1}));
        cluster_master_->cluster_manager()->write_message(
            req.leader_id(), serialize_raft_message(msg));
        return;
      }
      ...
      if (!check_if_previous_log_in_sync(req.prev_log_term(),
                                         req.prev_log_index())) {
      ...
      if (commit_index_ < req.leader_commit() &&
          req.leader_commit() < (int)log_entries_.size()) {
      ...
      // msg.loadAppendEntriesResponse(AppendEntriesResponseType({cur_term_,
      // node_id_, 1, log_entries_.back().index}));
      cluster_master_->cluster_manager()->write_message(
          req.leader_id(), serialize_raft_message(msg));
      return;
    ...
    case RaftMessage::APPEND_ENTRIES_RESPONSE: {
      ...
      pending_append_reqs_[follower_id] =
          std::max(0, pending_append_reqs_[follower_id] - 1);

      if (resp.append_result_success()) {
        // received success!
        // This is very tricky, matched_index_ should be increasing only,
        // however in the scenario where one node crashed and lost all of its
        // data, then restarted, the matched_index_[node_id] for that node
        // should be allowed to rollback to a smaller value ... however it seems
        // allowing this would break the assumptions required to guarantee the
        // consensus of the Raft algorithm, because leader could no longer make
        // assumptions that majority nodes have committed an entry if any node
        // could lose committed data ...

        // version 1, matched index increases only, this causes some slowness
        // issues when a node could lose persisted data and recover as a clean
        // node
        // matched_index_[follower_id] = std::max(matched_index_[follower_id],
        // last_index_synced);
        // version 2, matched index is allowed to decrease, I prefer this one
        // for now but needs more testing and proof
        ...
        if (commit_index_ < resp.last_index_synced()) {
          ...
          if (count_synced_nodes + 1 > total_nodes_ / 2) {
            LOG_DEBUG
                << "index " << resp.last_index_synced()
                << " is synced among majority nodes, will commit this index.\n";
            commit_log_entries(
                commit_index_ + 1,
                resp.last_index_synced() + 1);  // left close right open
            commit_index_ = std::max(commit_index_, resp.last_index_synced());
            cluster_node_storage_p_->store_log_entries_and_metadata(
                log_entries_, commit_index_, resp.last_index_synced() + 1,
                {commit_index_});
          } else {
            LOG_DEBUG << "index " << resp.last_index_synced()
                      << " is not synced among majority nodes yet, will not "
                         "commit this index.\n";
          }
        }
    ...
    case RaftMessage::REQUEST_VOTE_REQUEST: {
      ...
      if (invalid_request) {
        ...
        cluster_master_->cluster_manager()->write_message(
            candidate_node_id, serialize_raft_message(msg));
        return;
      }
    ...
    case RaftMessage::CLIENT_PUT_MESSAGE: {
      if (state_ != LEADER) {
        LOG_ERROR << "ERROR! current node is not leader! only leader accepts "
                     "put message reqeust.\n";
        return;
      }
      ...
      // LogEntry entry({(int)log_entries_.size(), cur_term_, random_id,
      // LogEntry::ADD, req.message});
    ...
    case RaftMessage::CLIENT_OPEN_QUEUE: {
      if (state_ != LEADER) {
        ...
        *resp.mutable_error_message() = "Node is not leader.";
        auto leader_address =
            cluster_master_->get_address_for_node(cur_leader_);
        ...
        *resp.mutable_leader_port() = leader_address.second;
        ...
        // TODO this only works when one client is opening in the queue in
        // retrying sync mode. we need client id or request id in the message to
        // support request/respnose style communication.
        cluster_master_->client_manager()->deliver_one_message_round_robin(
            partition_id_, serialize_raft_message(raft_msg));
        return;
      }
      RaftMessage raft_msg;
      ClientOpenQueueResponseType resp;
    ...
    case RaftMessage::CLIENT_COMMIT_MESSAGE: {
      if (state_ != LEADER) {
        LOG_ERROR << "ERROR! current node is not leader! only leader accepts "
                     "commit message reqeust.\n";
        return;
      }

      const ClientCommitMessageType& req =
          raft_msg.get_commit_message_request();
      LogEntry entry;
      entry.set_index((int)log_entries_.size());
      entry.set_term(cur_term_);
      ...
      // LogEntry entry({(int)log_entries_.size(), cur_term_, req.message_id,
      // LogEntry::COMMIT, ""});
      add_log_entry(std::move(entry));
      break;
    }
...
// including the heartbeat check response, and any response from appending a new
// log. Each error append entry response will trigger this, so there could be
// multiple series of req/resp to sync with a follower ... There should be only
// one, otherwise the process takes unnecessarily long time.
void ClusterNode::trigger_entry_update(int follower_id) {
  ...
  if (index_to_send >= 1 && index_to_send <= (int)log_entries_.size() - 1) {
    ...
    // TODO: reset MAX_PENDING_APPEND when peer is disconnected or gradually
    // reduce it over time.
    if (pending_append_reqs_[follower_id] < MAX_PENDING_APPEND) {
      cluster_master_->cluster_manager()->write_message(
          follower_id, serialize_raft_message(msg));
      pending_append_reqs_[follower_id]++;
    }
...
bool ClusterNode::check_if_previous_log_in_sync(int prev_log_term,
                                                int prev_log_index) {
...
void ClusterNode::clean_log_entries_out_of_sync(int index_to_remove) {
  log_entries_.erase(log_entries_.begin() + index_to_remove,
                     log_entries_.end());
}
...
void ClusterNode::commit_log_entries(int start_entry_index,
...
void ClusterNode::trigger_message_delivery() {
  ...
  for (auto message_id : id_temp) {
    ...
    // msg.loadServerSendMessageRequest(ServerSendMessageType{message_id,
    // message_queue_.get_message(message_id)});
    LOG_INFO << "delivering message to client : " << msg.DebugString() << '\n';
    int consumer_id =
        cluster_master_->client_manager()->deliver_one_message_round_robin(
            partition_id_, serialize_raft_message(msg));
    if (consumer_id == -1) {
      LOG_ERROR << "ERROR: unable to deliver message " << message_id
                << " to consumer!\n";
      return;
    }
