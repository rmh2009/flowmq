...
int ClientManager::deliver_one_message_round_robin(PartitionIdType partition_id, Message msg){
    ...
    size_t index_in_array = deliver_count_[partition_id] % consumer_client_id_array_[partition_id].size();
    ...
    acceptor_.async_accept([this](boost::system::error_code error, tcp::socket socket){
            ...
            new_session -> register_disconnect_handler([this, id](){
                    ...
                    for(auto& p : consumer_client_id_array_){
                      auto to_remove_from_array = std::find(p.second.begin(), p.second.end(), id);
