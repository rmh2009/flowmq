...
void ClusterManager::connect(int endpoint_id) {
  ...
  boost::asio::async_connect(
      *socket, endpoint_id_map_[endpoint_id],
      [this, socket, endpoint_id](const boost::system::error_code& ec,
                                  tcp::endpoint) {
        if (!ec) {
          ...
          session->register_handler([](const Message& msg) {
            LOG_ERROR << "Not supposed to get incoming message from this "
                         "session! messag:"
          ...
          session->register_disconnect_handler([this, endpoint_id]() {
            LOG_INFO
                << "Remote session closed, exiting now, retry in 2 seconds";
            auto timer =
                std::make_shared<boost::asio::deadline_timer>(io_context_);
            timer->expires_from_now(boost::posix_time::seconds(2));
            timer->async_wait(
                [timer, endpoint_id, this](boost::system::error_code const&) {
                  connect(endpoint_id);
                });
          });
...
void ClusterManager::accept_new_connection() {
  acceptor_.async_accept([this](boost::system::error_code error,
    ...
    if (!error) {
      ...
      new_session->register_handler(
          [this](const Message& msg) { handler_(msg); });
