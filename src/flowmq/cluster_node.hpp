...
class ClusterNode {
 public:
  const int HEARTBEAT_EXPIRE_SECONDS =
      3;  // if no heartbeat received in these seconds, will start new vote
  const int HEARTBEAT_CHECK_INTERVAL =
      5;                          // schedule interval to check heartbeats
 ...
 private:
  ...
  // Takes a container of entries, the API of C should be similar to
  // std::vector. This works for protobuf::RepeatedPtrField<> as well.
  template <class C>
  void append_log_entries(int last_log_index, const C& new_entries);

  // Serialize RaftMessage into lower level Message before sending to transport
  // layer Will add the correct partition id as well.
  Message serialize_raft_message(RaftMessage& raft_message);

  // ---------------- Persisting state of the log entries
  // store log entry state to disk
  void store_log_entries_and_commit_index(int start_entry_index,
  ...
  // state for pending append request. Do not send a append request if the
  // number of pending requests exceeds certain value.
  std::map<int, int>
      pending_append_reqs_;  // amp from node_id to pending append req.
