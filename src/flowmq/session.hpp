...
// Exposes APIs for writing and reading messages, and registering message
// handler. All write and read operations are async.
//
...
class Session : public std::enable_shared_from_this<Session> {
 public:
  ...
  void read_msg() { read_header(); }

  // register read message handler
  void register_handler(const ReadHandler& handler) { msg_handler_ = handler; }
