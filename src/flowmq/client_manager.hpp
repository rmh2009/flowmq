...
class ClientManagerInterface {
 public:
  ...
  virtual void register_disconnect_handler(
      const ClientDisconnectedHandler& handler) = 0;

  // Returns the client id the message was delivered to. Returns -1 on failure.
  virtual int deliver_one_message_round_robin(PartitionIdType partition_id,
                                              Message msg) = 0;
...
class ClientManager : public ClientManagerInterface {
 public:
  ...
  void register_disconnect_handler(
      const ClientDisconnectedHandler& handler) override;

  // Returns the client id the message was delivered to. Returns -1 on failure.
  int deliver_one_message_round_robin(PartitionIdType partition_id,
