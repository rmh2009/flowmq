...
class Message {
 public:
  ...
  enum { HEADER_LENGTH = 20, MAX_BODY_LENGTH = 1000 };

  const char* body() const { return &data_buffer_.data()[HEADER_LENGTH]; }

  char* body() { return &data_buffer_[HEADER_LENGTH]; }

  const char* header() const { return data_buffer_.data(); }
