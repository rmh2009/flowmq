...
{
    cluster_manager_p_->register_handler(std::bind(&ClusterMaster::message_handler, this, std::placeholders::_1));
    cluster_manager_p_->start();
    client_manager_p_->register_handler(std::bind(&ClusterMaster::message_handler, this, std::placeholders::_1));
    client_manager_p_->register_disconnect_handler(std::bind(&ClusterMaster::client_disconnected, this, std::placeholders::_1));
...
void ClusterMaster::add_cluster_node(
    ...
    LOG_INFO << "adding node with partition " << partition_id 
        << " and node_id " << node_id << " to cluster master\n";
...
void ClusterMaster::message_handler(const Message& msg){
    ...
    if(nodes_.find(raft_msg.partition_id()) == nodes_.end()){
        LOG_ERROR << "Parition id " << raft_msg.partition_id() << 
            " not found!\n";
        assert(false);
    }
...
std::pair<std::string, std::string> ClusterMaster::get_address_for_node(int node_id){
