...
int ClusterNodeStorage::load_log_entry_from_file(
    std::vector<LogEntry>* entries) {
  return LogEntryStorage::load_log_entry_from_file(storage_log_entry_filename_,
                                                   entries);
}

int ClusterNodeStorage::load_metadata_from_file(LogEntryMetaData* metadata) {
  return MetadataStorage::load_metadata_from_file(storage_metadata_file_name_,
                                                  metadata);
}

void ClusterNodeStorage::store_log_entries_and_metadata(
    const std::vector<LogEntry>& log_entries, int start_entry_index,
    int stop_entry_index, const LogEntryMetaData& metadata) {
...
void ClusterNodeStorage::consume_and_store_messages() {
  while (!cancelled_) {
    {
      std::unique_lock<std::mutex> lock(mutex_);
      LogEntryStorage::append_log_entry_to_file(storage_log_entry_filename_,
                                                log_entry_buffers_);
      MetadataStorage::save_metadata_to_file(storage_metadata_file_name_,
                                             metadata_buffer_);
