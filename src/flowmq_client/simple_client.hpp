...
class SimpleClient {
 public:
  using Handler = std::function<void(std::string message,MessageIdType message_id)>;
  ...
  SimpleClient(long long partition_id, boost::asio::io_context& io_context,
    ...
    stats();
    generic_client_.register_handler(
        std::bind(&SimpleClient::handle_message, this, std::placeholders::_1));
  }
  ...
  void commit_message(MessageIdType message_id) {
  ...
  void register_handler(const Handler& handler) { handler_ = handler; }
 ...
 private:
  ...
  void handle_message(const Message& msg) {
    LOG_INFO << "Received message : "
             << RaftMessage::deserialize_from_message(msg).DebugString()
             << '\n';
    RaftMessage raft_msg =
        RaftMessage::deserialize(std::string(msg.body(), msg.body_length()));
    ...
    if (raft_msg.type() == RaftMessage::CLIENT_OPEN_QUEUE_RESPONSE) {
      ...
      if (resp.status() == RaftMessage::ERROR) {
        ...
        generic_client_.reset_endpoint(
            reslv.resolve(resp.leader_ip(), resp.leader_port()));
